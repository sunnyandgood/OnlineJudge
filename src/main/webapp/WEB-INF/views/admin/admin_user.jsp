...
<script>
    ...
    window.caocuoEvents = {
        'click .update': function (e, value, row) {
            layer.open({
                type: 2,
                area: ['800px', '350px'],
                content: '${ctx}/page/userUpdate/' + row['userId'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
            });
        },
        'click .remove': function (e, value, row) {
            if(confirm('是否删除')){
                $.post('${ctx}/user/delete/' + row['userId'],function(r){
    ...
    function add(){
        layer.open({
            type: 2,
            area: ['800px', '500px'],
            content: '${ctx}/page/userAdd/' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }
    ...
    function addFromExcel() {
        layer.open({
            type: 2,
            area: ['500px', '350px'],
            content: ['${ctx}/page/userUp/', 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
    }

    function addToExcel() {
        $.post('${ctx}/user/addToExcel',function (r) {
            layer.msg(r.message);
        });
    }
</script>
